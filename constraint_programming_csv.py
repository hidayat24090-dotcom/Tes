...
def buat_jadwal():
    ...
    if status == cp_model.OPTIMAL or status == cp_model.FEASIBLE:
        print("Jadwal berhasil dibuat dan disimpan ke jadwal.csv")
        
        sorted_mk = sorted(mata_kuliah_aktif, key=lambda x: solver.Value(jadwal_vars[x.id]['start']))
        
        with open('jadwal.csv', 'w', newline='', encoding='utf-8') as csvfile:
            writer = csv.writer(csvfile)
            writer.writerow(['id', 'mata_kuliah_id', 'ruang_kelas_id', 'hari', 'jam_mulai', 'jam_selesai'])
            ...
            for idx, mk in enumerate(sorted_mk, 1):
                ...
                writer.writerow([idx, mk.id, ruang_layak[ruang_idx].id, HARI[day_idx], jam_mulai, jam_selesai])
